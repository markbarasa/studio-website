...
// Add to your types in the dashboard
type Booking = {
  id: number;
  name: string;
  phone: string;
  ...
  assignedStaff?: number[]; // ← ADD THIS: Array of staff IDs
};
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setBookings(Array.isArray(cleaned) ? cleaned : []);
    } catch (error) {
      console.error("Failed to load bookings:", error);
      addToast("Failed to load bookings data", "error");
      setBookings([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const updateStatus = useCallback(
    (id: number, status: BookingStatus) => {
      ...
      if (status === "confirmed") {
        ...
        if ((booking.deposit || 0) < requiredDeposit) {
          addToast(
            `Cannot confirm: Deposit of KSh ${requiredDeposit.toLocaleString()} required. Current deposit: KSh ${(booking.deposit || 0).toLocaleString()}`,
            "error",
          );
          return;
        }
      ...
      const updated = bookings.map((b) => {
        if (b.id === id) {
          ...
          if (status === "completed") {
            ...
            updatedBooking.deposit = b.price;
          }
    ...
    [bookings, persistBookings, addToast],
  );
  ...
  const updateDeposit = useCallback(
    (id: number, value: number) => {
      ...
      if (value < 0) {
        addToast("Deposit cannot be negative", "error");
        return;
      }

      if (value > booking.price) {
        addToast("Deposit cannot exceed total price", "error");
        return;
      }
      ...
      const updated = bookings.map((b) => {
        if (b.id === id) {
          const paymentStatus: PaymentStatus =
            value >= b.price ? "paid" : value > 0 ? "partial" : "unpaid";

          let newStatus = b.status;
          const requiredDeposit = Math.ceil(b.price / 2);
          if (b.status === "pending" && value >= requiredDeposit) {
            newStatus = "confirmed";
            addToast(
              `Deposit complete! Booking #${id} automatically confirmed.`,
              "success",
            );
          }

          return { ...b, deposit: value, paymentStatus, status: newStatus };
        }
        return b;
      });
      setBookings(updated);
    ...
    (
      ...
      const updated = bookings.map((b) => {
        if (b.id === bookingId) {
          ...
          const newDeposit = (b.deposit || 0) + amount;
          const paymentStatus: PaymentStatus =
            newDeposit >= b.price
              ? "paid"
              : newDeposit > 0
          ...
          let newStatus = b.status;
          const requiredDeposit = Math.ceil(b.price / 2);
          if (b.status === "pending" && newDeposit >= requiredDeposit) {
            newStatus = "confirmed";
            addToast(
              `Deposit complete! Booking #${bookingId} automatically confirmed.`,
              "success",
            );
          }
      ...
      persistBookings(updated);
      addToast(
        `Payment of KSh ${amount.toLocaleString()} recorded successfully`,
        "success",
      );
    },
    [bookings, persistBookings, addToast],
  ...
  const sendReminder = useCallback(
    (booking: Booking) => {
      const message = `Hi ${booking.name}, this is a reminder that your ${booking.service} event is on ${formatDate(booking.date)}. Please let us know if you have any questions.`;
      window.open(
  ...
  // Add this function in your AdminDashboard component with other action functions

  const handleEquipmentAssignment = useCallback(
    (bookingId: number, equipmentIds: number[]) => {
      // Update booking
      const updatedBookings = bookings.map((b) => {
        if (b.id === bookingId) {
          return { ...b, assignedEquipment: equipmentIds };
        }
        return b;
      });
      setBookings(updated);
      ...
      try {
        const storedEquipment = JSON.parse(
        ...
        const updatedEquipment = storedEquipment.map((eq: any) => {
          if (equipmentIds.includes(eq.id)) {
            return {
              ...eq,
              status: "in-use",
              assignedTo: bookingId,
              assignedToName: booking?.name || "",
            };
  ...
  const stats = useMemo(() => {
    ...
    const collectedAmount = bookings.reduce(
      (sum, b) => sum + (b.deposit || 0),
      0,
    );
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-zinc-400 mt-1">
                Manage bookings, payments, staff & clients
              </p>
            </div>
  ...
  className="px-4 py-2 rounded-lg transition text-sm font-bold bg-amber-500 text-black hover:bg-amber-400 shadow-lg shadow-amber-500/30"
>
  📋 Assign Staff & Equipment
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 mb-8">
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-3">
            ...
            <p className="text-lg font-bold">{stats.todayBookings}</p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-3">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-3">
            ...
            <p className="text-lg font-bold text-green-400">
              KSh {(stats.totalEarnings / 1000).toFixed(0)}K
            </p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-3">
            <p className="text-zinc-400 text-xs">Collected</p>
            <p className="text-lg font-bold text-green-400">
              KSh {(stats.collectedAmount / 1000).toFixed(0)}K
            </p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-3">
            <p className="text-zinc-400 text-xs">Pending $</p>
          ...
          <>
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <input
                ...
                className="flex-1 bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-2 text-white placeholder-zinc-500 focus:outline-none focus:border-zinc-600"
              />
              <select
                value={statusFilter}
                onChange={(e) =>
                ...
                className="bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-zinc-600"
              >
            ...
            {Object.keys(filteredAndGrouped).length === 0 ? (
              ...
              Object.keys(filteredAndGrouped)
                ...
                .map((month) => {
                  ...
                  const monthlyPending = data
                    .filter((b) => b.status !== "cancelled")
                    .reduce(
                      (sum, b) => sum + ((b.price || 0) - (b.deposit || 0)),
                      0,
                    );
                      ...
                      <div className="bg-zinc-900 p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm">
                          <span className="text-green-400">
                            💰 Earned: KSh {monthlyTotal.toLocaleString()}
                          </span>
                          ...
                          <span className="text-red-400">
                            ⚠️ Pending: KSh {monthlyPending.toLocaleString()}
                          </span>
                            ...
                            <tr>
                              {[
                                ...
                                <th
                                  ...
                                  className="p-3 cursor-pointer hover:bg-zinc-800 transition-colors select-none"
                                >
                          ...
                          <tbody>
                            {data.map((b) => {
                              ...
                              return (
                                <tr
                                  key={b.id}
                                  className={`border-t border-zinc-800 hover:bg-zinc-900/50 transition-colors ${isOverdue ? "bg-red-900/10" : ""}`}
                                >
                                  ...
                                  <td className="p-3">
                                    <input
                                      ...
                                      className="w-24 bg-zinc-800 border border-zinc-700 p-2 rounded focus:outline-none focus:border-zinc-500"
                                      min="0"
                                      max={b.price}
                                    />
                                  </td>
                                  ...
                                  <td className="p-3">
                                    <select
                                      ...
                                      className={`p-2 rounded text-sm font-medium ${b.status === "completed" ? "bg-green-900/50 text-green-400 border border-green-800" : b.status === "confirmed" ? "bg-blue-900/50 text-blue-400 border border-blue-800" : b.status === "cancelled" ? "bg-red-900/50 text-red-400 border border-red-800" : "bg-yellow-900/50 text-yellow-400 border border-yellow-800"}`}
                                    >
                                      <option value="pending">Pending</option>
                                  ...
                                  <td className="p-3">
                                    <span
                                      className={`px-2 py-1 rounded text-xs font-medium ${b.paymentStatus === "paid" ? "bg-green-900/50 text-green-400" : b.paymentStatus === "partial" ? "bg-yellow-900/50 text-yellow-400" : "bg-red-900/50 text-red-400"}`}
                                    >
                                  ...
                                  <td className="p-3">
                                    <div className="flex gap-1">
                                      ...
                                      <button
                                        ...
                                        className="bg-green-500 hover:bg-green-600 text-black px-2 py-1 rounded text-xs transition"
                                        title="WhatsApp"
                                      >
                                        💬
                                      </button>
                                      ...
                                      {showDeleteConfirm === b.id ? (
                                        <div className="flex gap-1">
                                          ...
                                          <button
                                            ...
                                            className="bg-zinc-600 hover:bg-zinc-500 text-white px-2 py-1 rounded text-xs"
                                          >
                                            ✕
                                          </button>
                                        </div>
                                        ...
                                        <button
                                          ...
                                          className="bg-zinc-700 hover:bg-red-600 text-white px-2 py-1 rounded text-xs transition"
                                          title="Delete"
                                        >
                                          🗑
                                        </button>
                            ...
                            <tr className="bg-zinc-900/80 font-bold">
                              ...
                              <td className="p-3 text-red-400">
                                KSh {monthlyPending.toLocaleString()}
                              </td>
                              <td colSpan={3}></td>
                            </tr>
                          </tbody>
      ...
      {showPaymentModal && (
        <PaymentModal
          ...
          onPaymentComplete={(amount, method, reference) => {
            handlePayment(showPaymentModal.id, amount, method, reference);
      ...
      {showBookingDetail && selectedBooking && (
        ...
        >
          <div
            className="bg-zinc-900 border border-zinc-700 rounded-xl p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto"
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 bg-zinc-800/50 rounded-xl p-4">
                  <div>
                    <p className="text-zinc-400 text-xs">Client Name</p>
                    <p className="font-semibold">{selectedBooking.name}</p>
                  </div>
                  <div>
                    <p className="text-zinc-400 text-xs">Phone Number</p>
                  ...
                  <div>
                    <p className="text-zinc-400 text-xs">Service</p>
                    <p className="font-semibold">{selectedBooking.service}</p>
                  </div>
                  <div>
                    <p className="text-zinc-400 text-xs">Package</p>
                  ...
                  <div>
                    ...
                    <p className="font-semibold">
                      {selectedBooking.venue || "Not specified"}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-zinc-400 text-xs">Payment Status</p>
                    <span
                ...
                <div className="bg-zinc-800 rounded-xl p-4">
                  ...
                  <div className="mb-3">
                    ...
                    <div className="h-3 bg-zinc-700 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(((selectedBooking.deposit || 0) / selectedBooking.price) * 100, 100)}%`,
                        }}
                      />
                    </div>
                  </div>
                  ...
                  {(selectedBooking.price || 0) -
                    ...
                    <button
                      ...
                      className="w-full mt-4 bg-green-600 hover:bg-green-700 py-2.5 rounded-lg font-medium transition flex items-center justify-center gap-2"
                    >
                      💳 Record Payment
                    </button>
                  )}
              ...
              {/* EQUIPMENT ASSIGNMENT - THIS IS THE KEY SECTION */}
              <div>
                <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-3">
                  🎬 Equipment Assignment
                </h3>
                <div className="bg-zinc-800/50 rounded-xl p-4">
                  {/* Simple equipment assignment UI */}
                  <EquipmentQuickAssign
                    bookingId={selectedBooking.id}
                    bookingDate={selectedBooking.date}
                    currentAssigned={selectedBooking.assignedEquipment || []}
                    onAssign={(equipmentIds) => {
                        ...
                        >
                          <div>
                            ...
                            <p className="text-xs text-zinc-400">
                              {payment.method.toUpperCase()} • Ref:{" "}
                              {payment.reference}
                            </p>
                          </div>
                ...
                {editingNotes === selectedBooking.id ? (
                  <div className="space-y-2">
                    <textarea
                      ...
                      className="w-full bg-zinc-800 border border-zinc-700 rounded-lg p-3 text-sm focus:border-zinc-500 transition resize-none"
                      rows={4}
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          updateNotes(selectedBooking.id, notesText)
                        }
                  ...
                  <div
                    ...
                    className="bg-zinc-800 rounded-lg p-4 cursor-pointer hover:bg-zinc-700 transition min-h-[60px]"
                  >
              ...
              {/* Currently Assigned Equipment Chips */}
              {selectedBooking.assignedEquipment &&
                selectedBooking.assignedEquipment.length > 0 && (
                  <div>
                    <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-3">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() =>
                      window.open(
                        `https://wa.me/${selectedBooking.phone.replace(/\D/g, "")}`,
                        "_blank",
                      )
                    }
                    className="bg-green-600 hover:bg-green-700 py-3 rounded-lg font-medium transition flex items-center justify-center gap-2"
                  >
                    💬 WhatsApp
                  </button>
                  <button
                    ...
                    className="bg-purple-600 hover:bg-purple-700 py-3 rounded-lg font-medium transition flex items-center justify-center gap-2"
                  >
                    📄 Generate Contract
                  </button>
                  <button
                    onClick={() => {
                      sendReminder(selectedBooking);
                    }}
                    className="bg-blue-600 hover:bg-blue-700 py-3 rounded-lg font-medium transition flex items-center justify-center gap-2"
                  >
                    🔔 Send Reminder
                  </button>
                  <button
                    onClick={() => {
                      setShowDeleteConfirm(selectedBooking.id);
                      setShowBookingDetail(false);
                    }}
                    className="bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white py-3 rounded-lg font-medium transition flex items-center justify-center gap-2"
                  >
                    🗑 Delete Booking
                  </button>
                </div>
